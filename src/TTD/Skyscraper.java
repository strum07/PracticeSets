...
public class Skyscraper {
    ...
    public static void main(String args[]) {
        ...
        int [][] buildings1 = {{1,11,5}};

        Skyscraper sd = new Skyscraper();
        List<int[]> criticalPoints = sd.getSkyline(buildings);
        criticalPoints.forEach(cp -> System.out.println(cp[0] + " " + cp[1]));


        Skyscraper as = new Skyscraper();
        List<int[]> criticalPoints1 = as.getSkyline(buildings1);
        criticalPoints1.forEach(cp -> System.out.println(cp[0] + " " + cp[1]));
